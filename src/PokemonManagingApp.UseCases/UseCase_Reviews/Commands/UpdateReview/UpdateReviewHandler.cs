...
public class UpdateReviewHandler(IUnitOfWork unitOfWork) : IRequestHandler<UpdateReviewCommand, Result>
{
    ...
    public async Task<Result> Handle(UpdateReviewCommand request, CancellationToken cancellationToken)
    {
        Review? checkingReview =
            await _unitOfWork.ReviewRepository.GetEntityByConditionAsync(r => r.Id.Equals(request.Id), true);
        if (checkingReview is null) return Result.NotFound("Review not found");
        Pokemon? checkingPokemon =
            await _unitOfWork.PokemonRepository.GetEntityByConditionAsync(p => p.Id.Equals(request.PokemonId), false);
        ...
        return Result.SuccessWithMessage("Review updated successfully");
    }
}