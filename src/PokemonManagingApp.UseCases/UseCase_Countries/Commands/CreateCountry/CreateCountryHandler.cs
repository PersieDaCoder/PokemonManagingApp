    ...
    public async Task<Result<CountryDTO>> Handle(CreateCountryCommand request, CancellationToken cancellationToken)
    {
        Country? checkedCountry =
            await _unitOfWork.CountryRepository
                .GetEntityByConditionAsync(c => c.Name.Equals(request.Name), false);
        if (checkedCountry is not null)
            return Result<CountryDTO>.Error("Country already exists");
        ...
        return Result.Success(CountryMapper.MapToDTO(addingCountry),"Country is created successfully");
    }
}